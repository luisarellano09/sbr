...
import { SET_ESP32_MODE_NODE1_PROGRAM, SET_ESP32_MODE_NODE1_RESTART } from "@/graphql/mutations";
...
export default function ControlESP32Node01() {

    const [nodeRestart] = useMutation(SET_ESP32_MODE_NODE1_RESTART);
    const [nodeProgram] = useMutation(SET_ESP32_MODE_NODE1_PROGRAM);
    ...
    const HandleSendHostRequest= (request)=>{
        ...
        switch(request){
            case "RESTART": 
                nodeRestart({
                    onCompleted: ()=> toast.success("Command ESP32 Node01: Restart"),
                    onError: (e)=> {
            ...
            case "PROGRAM": 
                nodeProgram({
                    onCompleted: ()=> toast.success("Command ESP32 Node01: Program"),
                    onError: (e)=> {
                        let networkErrorMessage = "";
                        try{
                            networkErrorMessage = e.networkError.result.errors[0].message;
	...
	return (
		<div className="w-full max-w-[260px] border-medium px-1 py-2 rounded-small border-[#646364]">
            <Listbox variant="faded" aria-label="Host Request" onAction={(key)=> HandleSendHostRequest(key)}>
                <ListboxSection title="ESP32 Node01" showDivider className="text-[#e43f3f]">
