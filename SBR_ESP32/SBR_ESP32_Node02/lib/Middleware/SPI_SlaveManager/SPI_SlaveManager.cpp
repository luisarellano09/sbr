...
SPI_SlaveManager::SPI_SlaveManager(uint8_t _SO, uint8_t _SI, uint8_t _SCLK, uint8_t _SS){
    ...
    if (CleanBuffer() != RC_e::SUCCESS){
        Debug("Error: CleanBuffer in SPI_SlaveManager::SPI_SlaveManager()");
    }

    if (SPIConfigure(_SO, _SI, _SCLK, _SS) != RC_e::SUCCESS){
        Debug("Error: CleanBuffer in SPI_SlaveManager::SPI_SlaveManager()");
    }
}
...
RC_e SPI_SlaveManager::ListenRequest(){
    ...
    if (length >= SPI_MANAGER_REQUEST_SIZE && digitalRead(m_SS) == HIGH){
        ...
        for (int i=0; i<length/SPI_MANAGER_REQUEST_SIZE; i++){
            ...
            // Calculate offset
            int _offset = SPI_MANAGER_REQUEST_SIZE*i;
            ...
            if ((retCode = HandleRequest(&_bufferOneRequest)) != RC_e::SUCCESS){
                Debug("Error: HandleRequest in SPI_SlaveManager::ListenRequest()");
                return retCode;
            }
        }
    }
...
//=====================================================================================================
RC_e SPI_SlaveManager::AddWriteRequest(COM_REQUEST_REG_ID_e regId, int32_t data){
    ...
    if (m_RequestsArray == NULL){
        Debug("Error: NULL_POINTER in SPI_SlaveManager::AddWriteRequest()");
        return RC_e::ERROR_NULL_POINTER;
    }

    // Add Write Request to the buffer
    if ((retCode = AddRequest(COM_REQUEST_TYPE_e::WRITE, regId, data)) != RC_e::SUCCESS){
        Debug("Error: AddRequest in SPI_SlaveManager::AddWriteRequest()");
        return retCode;
    }
...
//=====================================================================================================
RC_e SPI_SlaveManager::EnableDebugMode(){
    this->m_debugMode = 1;
    return RC_e::SUCCESS;
}

//=====================================================================================================
RC_e SPI_SlaveManager::DisableDebugMode(){
    this->m_debugMode = 0;
    return RC_e::SUCCESS;
}
...
RC_e SPI_SlaveManager::HandleRequest(array_t* buffer){
    ...
    if (buffer == NULL){
        Debug("Error: NULL_POINTER in SPI_SlaveManager::HandleRequest()");
        return RC_e::ERROR_NULL_POINTER;
    }
    ...
    if ((retCode = BufferToRequest(_buffer, &request)) != RC_e::SUCCESS){
        Debug("Error: BufferToRequest in SPI_SlaveManager::HandleRequest()");
        return retCode;
    }

    // Check CRC
    if (request.CRC != CalculateCrcFromRequest(&request)){
        Debug("Error: CalculateCrcFromRequest in SPI_SlaveManager::HandleRequest()");
    ...
    switch(request.comRequestType){
        case COM_REQUEST_TYPE_e::WRITE:
            if ((retCode = HandleWriteRequest(&request)) != RC_e::SUCCESS){
                Debug("Error: HandleWriteRequest in SPI_SlaveManager::HandleRequest()");
                return retCode;
            }
            break;
        ...
        case COM_REQUEST_TYPE_e::READ:
            if ((retCode = HandleReadRequest()) != RC_e::SUCCESS){
                Debug("Error: HandleReadRequest in SPI_SlaveManager::HandleRequest()");
                return retCode;
            }
            break;
    }
...
RC_e SPI_SlaveManager::HandleWriteRequest(COM_REQUEST_st* request){
    ...
    // Check if the pointer is null
    if (request == NULL){
        Debug("Error: NULL_POINTER in SPI_SlaveManager::HandleWriteRequest()");
        return RC_e::ERROR_NULL_POINTER;
    }

    if ((retCode = RegisterHandler((COM_REQUEST_REG_ID_e)request->comRequestRegId, request->data)) != RC_e::SUCCESS){
        Debug("Error: RegisterHandler in SPI_SlaveManager::HandleWriteRequest()");
        return retCode;
    }
...
//=====================================================================================================
RC_e SPI_SlaveManager::HandleReadRequest(){  // Todo: implement logic in case read is stopped, FIFO
    ...
    if (m_RequestsArrayIndexToSend <= m_RequestsArrayIndex){
        ...
        if ((retCode = CleanBuffer()) != RC_e::SUCCESS){
            Debug("Error: CleanBuffer in SPI_SlaveManager::HandleReadRequest()");
            return retCode;
        }
    ...
    if ((retCode = RequestToBuffer(&_request, _tempBuffer)) != RC_e::SUCCESS){
        Debug("Error: RequestToBuffer in SPI_SlaveManager::HandleReadRequest()");
        return retCode;
    }
...
//=====================================================================================================
RC_e SPI_SlaveManager::AddRequest(COM_REQUEST_TYPE_e type, COM_REQUEST_REG_ID_e regId, int32_t data){

    // Check if array is null
    if (m_RequestsArray == NULL){
        Debug("Error: NULL_POINTER in SPI_SlaveManager::AddRequest()");
        return RC_e::ERROR_NULL_POINTER;
    }
    ...
    // Serial.print("Req: ");
    // Serial.println(m_RequestsArray[m_RequestsArrayIndex].comRequestType);
    // Serial.print("ReqID: ");
    // Serial.println(m_RequestsArray[m_RequestsArrayIndex].comRequestRegId);
    // Serial.print("Data: ");
    // Serial.println(m_RequestsArray[m_RequestsArrayIndex].data);
    // Serial.print("CRC: ");
    // Serial.println(m_RequestsArray[m_RequestsArrayIndex].CRC);
...
//=====================================================================================================
RC_e SPI_SlaveManager::Debug(char* msg){
    if (this->m_debugMode){
        Serial.println(msg);
    }
